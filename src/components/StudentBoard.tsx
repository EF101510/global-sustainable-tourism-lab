...
import { ChevronUp, Pencil, RefreshCw, Trash2, X } from 'lucide-react';
...
export default function StudentBoard({
  ...
  // Lock the page behind the modal so touch drags don't scroll it through
  // the backdrop (iOS scroll chaining). Mounted only while open.
  useScrollLock();
  ...
  // On phones the submit form is collapsed by default so the posts list —
  // the thing students come to read — gets the bulk of the sheet; desktop
  // keeps it pinned open. Tapping the prompt re-expands it.
  ...
  const submit = async () => {
    ...
    try {
      ...
      setContent('');
      // On mobile, collapse the form so the freshly posted item shows at
      // the top of the now-roomy list.
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-6 bg-black/50 backdrop-blur-sm">
      <div className="glass-modal rounded-2xl shadow-2xl w-full max-w-2xl h-[88dvh] sm:h-auto max-h-[88dvh] sm:max-h-[85vh] flex flex-col text-white">
        ...
        <div className="px-4 sm:px-6 py-3 sm:py-4 border-b border-white/15 bg-white/5">
          {!formOpen ? (
            <button
              onClick={() => setFormOpen(true)}
              className="w-full flex items-center justify-center gap-2 text-sm font-medium text-cyan-200 bg-white/5 hover:bg-white/10 border border-white/15 rounded-lg px-3 py-2.5 transition"
            >
            ...
            <>
              <button
                ...
                className="sm:hidden w-full flex items-center justify-between text-sm font-medium text-white/80 mb-3"
              >
                <span className="flex items-center gap-2">
                  <Pencil className="w-4 h-4" /> Share your solution
                </span>
